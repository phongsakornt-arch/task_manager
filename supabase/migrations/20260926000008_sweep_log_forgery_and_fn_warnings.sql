-- ============================================================
-- Remaining bug-sweep items.
--
-- 1. activity_log forgery: activity_insert_all only checked that the
--    caller is logged in, so any user could insert a row with someone
--    else's actor_id/actor_email and fake the audit trail. A BEFORE
--    INSERT trigger now overwrites both with the real caller whenever
--    the insert comes from a logged-in user. Edge functions insert via
--    the service role (auth.uid() is null there), so their system rows
--    keep whatever actor they set.
--
-- 2. plpgsql_check warnings on the Data Master resolvers: both call
--    set_config() (VOLATILE) while declared STABLE, and the batch one
--    had an unused shadowed idx variable and an untyped '{}' literal.
-- ============================================================

CREATE OR REPLACE FUNCTION enforce_activity_log_actor()
RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF auth.uid() IS NOT NULL THEN
    NEW.actor_id := auth.uid();
    NEW.actor_email := (SELECT email FROM users WHERE id = auth.uid());
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS activity_log_enforce_actor ON activity_log;
CREATE TRIGGER activity_log_enforce_actor
  BEFORE INSERT ON activity_log
  FOR EACH ROW EXECUTE FUNCTION enforce_activity_log_actor();

ALTER FUNCTION resolve_master_member(text, text, text, text, text, text, text) VOLATILE;
