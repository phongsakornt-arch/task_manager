-- ============================================================
-- Let the caller pick which criteria to check per batch, instead of
-- always trying every tier (phone -> email -> national_id -> name
-- exact -> name fuzzy -> business exact -> business fuzzy) for every
-- row. The name/business pair in particular means every plain-name
-- row was already paying for a SECOND fuzzy trigram scan (business
-- name) that's almost never useful when the list is known to be just
-- people's names — this lets the UI skip that work entirely when the
-- user knows it doesn't apply, cutting real load on large batches.
-- Every flag defaults to true, so existing callers are unaffected.
-- ============================================================
...
-- overload เดิม (แค่ inputs) ลบทิ้ง กัน PostgREST เจอ ambiguous function
